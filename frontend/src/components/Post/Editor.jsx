...
const Editor = ({ closeModal }) => {
  ...
  const { mutate: createPost } = useMutation({
    mutationFn: async ({ title, author_id, topic_id, image_link, desc  }) => {
  ...
  const onSubmit = async (data) => {
    ...
    const payload = {
      ...
      image_link: blocks?.blocks?.find((block) => block.type === "image")?.data?.file?.url || null, // Extract the first image URL if available
      desc: blocks?.blocks?.find((block) => block.type === "paragraph")?.data?.text || "", // Extract a paragraph as description
    };
  ...
  return (
    <div className="w-full p-4 bg-zinc-50 rounded-lg border border-zinc-200">
      <form id="post-form" className="w-full" onSubmit={handleSubmit(onSubmit)}>
